...
<table cellpadding="0" cellspacing="0" border="0" class="tableBlue" width="100%">
	<tr>
		...
		<td nowrap="nowrap" class="tableThBlue" align="right">
			...
			{if 1||$active_worker->hasPriv('core.home.workspaces')}<span style="font-size:12px"> | </span><a href="javascript:;" onclick="genericAjaxGet('{$view->id}_tips','c=internal&a=viewShowCopy&view_id={$view->id}');toggleDiv('{$view->id}_tips','block');" class="tableThLink">{$translate->_('common.copy')|lower}</a>{/if}
