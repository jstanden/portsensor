					...
					<td nowrap="nowrap">
						{if !empty($source_manifests)}
							{foreach from=$source_manifests item=manifest key=manifest_id}
								&#187; <a href="javascript:;" onclick="genericAjaxGet('frmConfigFieldSource','c=setup&a=getFieldSource&ext_id={$manifest_id}');">{$manifest->name}</a><br>
							{/foreach}
						{/if}
					</td>
				</tr>
		...
		<td width="100%" valign="top">
			<form action="{devblocks_url}{/devblocks_url}" method="post" id="frmConfigFieldSource" onsubmit="return false;">
				{if !empty($ext_id)}
					...
					{include file="$core_tpl/setup/tabs/fields/edit_source.tpl" object=$source_manifest}
				{/if}
			</form>
		</td>
