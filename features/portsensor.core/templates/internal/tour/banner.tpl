	...
	<tr>
		<td colspan="3">
			...
			{foreach from=$tour.callouts item=callout name=callouts}
				<img src="{devblocks_url}c=resource&a=portsensor.core&f=images/help.gif{/devblocks_url}" align="absmiddle"> <a href="#{$callout->id}" onclick="genericAjaxPanel('c=internal&a=showCallout&id={$callout->id}','{$callout->id}',false);">{$callout->title}</a>
			{/foreach}
			</p>
			{/if}
		</td>
